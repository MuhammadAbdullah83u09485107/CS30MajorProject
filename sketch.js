...
function resetGame() {
  ...
  restartButton.hide();
  score = 0;
  console.log("Game reset. Birds after collision", birds);
}
...
### Key Changes:
1. **Corrected Collision Detection**: Added `bird.overlap(pig, destroyPig)` inside the `drawGame` function to ensure birds destroy pigs upon collision.
2. **Fixed Gravity Application**: Applied gravity correctly to non-static objects in the `applyGravity` function.
3. **Handled Bird Hitting the Ground**: Checked if the bird hits the ground and called `nextBird()` to handle bird transitions.
4. **Removed Pigs on Collision**: Updated the `destroyPig` function to ensure pigs are removed correctly and score is updated.

This should ensure that the pigs are properly destroyed when hit by the birds, they don't fall immediately, and they don't float.
